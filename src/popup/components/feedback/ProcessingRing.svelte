<script lang="ts">
	...
	async function getNextAlarm(id: string) {
		...
		return alarms
			.filter((alarm) => alarm.name === `remainder:${id}` || alarm.name.startsWith(`retry:${id}:`))
...
<div class="relative flex items-center justify-center w-[72px] h-[72px]">
	<svg width="72" height="72" class="absolute -rotate-90 pointer-events-none">
		...
		>
			<circle
				...
				cy="36"
				r="3"
				class="fill-white dark:fill-neutral-100 stroke-accent"
				stroke-width="1.5"
	...
	<span
		class="text-sm font-semibold text-neutral-800 dark:text-neutral-200 z-10 tabular-nums tracking-tight"
	>
		{currentLabel}
	</span>
</div>
