<script lang="ts">
	...
	let { id, title, message, enabled, interval, onToggle, onIntervalChange } = $props<{
		id: string;
		title: string;
		message: string;
		enabled: boolean;
...
>
	...
	<div
		...
		style="grid-template-rows: {expanded ? '1fr' : '0fr'}; opacity: {expanded
			? 1
			: 0}; margin-top: {expanded ? '1rem' : '0'};"
	>
